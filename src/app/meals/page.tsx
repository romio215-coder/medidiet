...
function FilterButton({ label, active, onClick }: { label: string, active: boolean, onClick: () => void }) {
    return (
        ...
        ${active
                    ? 'bg-[#6B8E23] text-white border-[#556B2F] shadow-sm'
                    : 'bg-[#FFFEF0] text-[#8D6E63] border-[#8D6E63] hover:bg-white'}
      `}
        >
            {label}
        </button>
...
function StatusBadge({ status, t }: { status: 'SAFE' | 'CAUTION' | 'DANGER', t: any }) {
    if (status === 'SAFE') return <span className="bg-[#6B8E23]/20 text-[#33691E] border border-[#6B8E23] px-2 py-1 rounded-lg text-xs font-bold">{t.filterSafe.replace('✅', '🍃')}</span>;
    if (status === 'CAUTION') return <span className="bg-[#E6B333]/20 text-[#827717] border border-[#E6B333] px-2 py-1 rounded-lg text-xs font-bold">{t.filterCaution.replace('⚠️', '🍄')}</span>;
    return <span className="bg-[#E07A5F]/20 text-[#BF360C] border border-[#E07A5F] px-2 py-1 rounded-lg text-xs font-bold">❌ AVOID</span>;
}
